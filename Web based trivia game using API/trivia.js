...
/*
The following variables are meant to keep track on size.
 */
var sizeIncrement = 7;
var selectedTest;

/*
Checks to see which test was selected.
Loads the questions for that test.
 */
...
/*
track variable to selected test
 */
...
/*
put all the question answers into an array
and shuffle the array to randomize the question placement
 */
...
/*
loads and writes all the questions and radio button to the html page
Since I did not use objects I am also keeping track of all buttons and questions by
using organized id's and value names.
 */
function loadTest(x){
	let group = 1;
	//Loops through test x and adds all questions and buttons
	for(let i = 0; i < tests["Test "+ x].length;i++) {
		let quest = [];
		let span = document.createElement("span");
		let list = document.createElement("ul");
		span.id = "q" + i;
		let question = tests["Test " + x][i].question;
		span.innerHTML = question + "<br>";
		questions.appendChild(span);
		list.id = "list"+i;
		span.appendChild(list);

		//shuffles the answers for question i
		for(let err = 0; err < tests["Test " + x][i].incorrect_answers.length; err++) {
			quest.push(tests["Test " + x][i].incorrect_answers[err]);
		}
		quest.push(tests["Test " + x][i].correct_answer);
		shuffle(quest);
		//Adds all the answers into a ul li
		for(let a = 0; a < quest.length; a++){
			let li = document.createElement("li");
			li.innerHTML = (a+1)+ ". " + quest[a];
			list.appendChild(li);
		}
		//Creates all the buttons
		let buttonDiv = document.createElement("div");
		buttonDiv.id = "div"+i;
		for(let err = 0; err < quest.length; err++) {
			let check = document.createElement("input");
			let numbered = document.createElement("span");
			numbered.innerHTML = " " +(err+1)+". ";
			check.setAttribute("type", "radio");
			check.id = "b" + i + "-" + err;
			check.value = quest[err];
			check.name = i;
			check.className = "test";
			numbered.appendChild(check);
			buttonDiv.appendChild(numbered);
		}
		span.appendChild(buttonDiv);
	}
}
//Resets the red text colour
function resetColour(){
	for(let i = 0; i < tests["Test "+ selectedTest].length;i++) {
		document.getElementById("q"+i).style.background = "";
	}
}
...
function calculateScore(){
	...
	let right = 0;
	let wrong = 0;
	//Loops through all buttons, and if the button is right, increment the variable right else increment wrong
	for(let i = 0; i < tests["Test "+ selectedTest].length;i++) {
		let divButtons = document.getElementById("div"+i).querySelectorAll(".test").length;
		flag = false;
		for(let x = 0; x < divButtons; x++) {
			let b = document.getElementById("b"+i+"-"+(x));
			if(b.checked == true){
				let divImg = document.getElementById("div"+i);
				let img = document.createElement("img");
				img.width = "20";
				if(b.value == tests["Test "+ selectedTest][i].correct_answer){
					flag = true;
	...
	result.innerText = right+ "/" +(right+wrong);
	result.scrollIntoView();

	//Play a sound effect whenever the score is calculated
	let celebrate = document.createElement("audio");
	celebrate.style.display = "none";
	celebrate.src = "sound/score.mp3";
	celebrate.play();
...
//Checks to see if all questions were answers else display them in red text colour.
function checkTest(){
	resetColour();
	let flag = false;
	for(let i = 0; i < tests["Test "+ selectedTest].length;i++) {
		...
		if(flag == false){
			document.getElementById("q"+i).style.background = "red";
		}
	}
	if(flag == false){
		alert("One or more questions are not answered");
	}
	else{
		calculateScore();
	}
}
//If user does not want to clear page remove div pop-up
function noClear(){
	let popUp = document.getElementById("popUp");
	popUp.style.display = 'none';
	questions.style.display = "block";
}
//If user wants to clear page. Then clear the page
function yesClear(){
	for(let x = 0; x  = questions.getElementsByTagName("*").length; x++){
...
//Requests JSON object from server. Add the object to the tests object and load all the questions
	...
	xhttp.onreadystatechange = function(){
		if(xhttp.readyState == 4){
			if(xhttp.status == 200) {
				...
				tests["Test " + sizeIncrement] = data;
				selectedTest = sizeIncrement;
				loadTest(sizeIncrement);
				sizeIncrement++;
			}
...
let loadB = document.getElementById("loadB").addEventListener("click",checkSelected);
let check = document.getElementById("check").addEventListener("click",checkTest);
let clear = document.getElementById("clear").addEventListener("click",clearTest);
let no = document.getElementById("no").addEventListener("click",noClear);
let yes = document.getElementById("yes").addEventListener("click",yesClear);
let random = document.getElementById("random").addEventListener("click",httpRequest);